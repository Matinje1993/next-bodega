        ...
        <div className={`relative ${readonly ? 'opacity-60' : 'opacity-100'} mt-[16px]`}>
            <div className="m-0 min-w-0 flex items-center">
                <input
                    ...
                    readOnly={readonly}
                    className={`px-3 pr-3 pt-[14px] pb-[14px] ${className} block w-full h-12 outline-none font-montserrat font-normal text-base leading-5 ${readonly ? 'text-[#747474]' : 'text-black'} border ${isLabelFloated && !readonly ? 'border-[#747474]' : 'border-[#dcdcdc]'} rounded`}
                    {...props}
                />
                {children}
            </div>
